...
| `tui.transcriptCommit`      | enum    | `settle`         | When a finalized transcript block retires into terminal scrollback: `settle` commits each block as soon as it can no longer change (ordinary CLI output — committed rows are immutable), `capacity` keeps blocks live, reflowing on resize, until the screen runs out of room. |
