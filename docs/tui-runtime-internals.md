...
Finalizing a later block never bypasses an active predecessor. `peekFinalizedBatch(width, capacity)` retires the shortest settled prefix that lets the remaining live tail fit `capacity`, stops at the first active block, and reoffers the same id until `acknowledgeFinalizedBatch()` succeeds. `peekSettledBatch(width)` retires that same prefix with no capacity gate at all — the `tui.transcriptCommit: settle` default, where a block leaves the viewport the moment it finalizes — and `peekFlushBatch(width)` takes the whole eligible prefix during graceful shutdown. Capacity retirement keeps the screen full: while it has room nothing retires during ordinary operation, so a submitted message is visible immediately and recent blocks keep reflowing on resize. Settle retirement instead leaves the viewport holding only active blocks and chrome, so its rows are immutable history from the moment they were printed.
